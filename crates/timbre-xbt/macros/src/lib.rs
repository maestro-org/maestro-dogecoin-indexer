    ...
    let encode_impl =
        match input.data {
            Data::Struct(data) => {
                ...
                let encoding = fields.iter().enumerate().map(|(i, f)| {
                    ...
                    if i == fields_len - 1 {
                        ...
                        // All other fields
                        quote! {
                            encoder = encoder.append_with_break(&self.#field_name);
                        }
                    }
                });
        ...
        Data::Struct(data) => {
            ...
            let field_decodes = fields.iter().enumerate().map(|(i, f)| {
                ...
                if i == fields_len - 1 {
                    ...
                    quote! {
                        let (#field_name, rest) = <#field_ty as Decode>::decode(bytes)?;
                        bytes = &rest[1..];  // Update the slice to the remaining bytes after decoding and skipping the break
                    }
                }
            });
