/*
   ...
   Creates reducer outputs to signal that a UTxO containing some runes was
   consumed or produced, along with the different kinds of runes and associated
   amounts.
*/
...
pub struct Output {
    pub utxo_hash: [u8; 32],
    pub utxo_index: u32,
    pub action: UtxoAction<Vec<((u64, u32), u128)>>, // list of (runeid, amount) pairs
}
...
impl Reducer {
    fn process_consumed_txo(
        ...
        if ctx.utxo_runes(input).is_some() {
            outputs.push(ReducerOutput::RunesByUtxo(Output {
    ...
    fn process_produced_txo(
        ...
        if let Some(runes) = ctx.utxo_runes(&outpoint) {
            ...
            outputs.push(ReducerOutput::RunesByUtxo(Output {
                ...
                action: UtxoAction::Produced(runes),
            }))
        }
...
impl Config {
    pub fn plugin(self) -> super::Reducer {
        ...
        super::Reducer::RunesByUtxo(reducer)
    }
}
