...
use bitcoin::{
    ...
    hashes::Hash,
    secp256k1, Block, BlockHash, VarInt,
};
use bitcoin::{
    consensus::{Encodable, ReadExt},
    network::{
        address,
...
fn build_version_message() -> NetworkMessage {
    ...
    let version = NetworkMessage::Version(message_network::VersionMessage {
        ...
        user_agent: "/compressor:0.1.0/".to_string(),
        start_height: 0,
        relay: false,
    });
    return version;
...
impl Request {
    ...
    // TODO: chunk hashes? (https://en.bitcoin.it/wiki/Protocol_documentation#getdata)
...
impl PeerHandler {
    ...
    async fn tick(&mut self) -> Result<(), Error> {
        select! {
            maybe_req = self.request_recv.recv() => {
                ...
                let to_send = message::RawNetworkMessage{ magic: self.magic, payload: to_send };
            ...
            maybe_msg = self.ingress_recv.recv() => {
                ...
                match msg {
                    ParsedNetworkMessage::Version(ver) => {
                        ...
                        self.egress_send.send(message::RawNetworkMessage{ magic:self.magic, payload: NetworkMessage::Verack }).await.map_err(|_| Error::EgressChannelClosed)?;
                    },
                    ...
                    ParsedNetworkMessage::Ping(nonce) => {
                        ...
                        self.egress_send.send(message::RawNetworkMessage { magic: self.magic, payload: NetworkMessage::Pong(nonce) }).await.map_err(|_| Error::EgressChannelClosed)?;
                    },
                    ParsedNetworkMessage::Headers(headers) => {
    ...
    pub async fn run(&mut self) -> Result<(), Error> {
        ...
        let version_message = message::RawNetworkMessage {
            magic: self.magic,
            payload: build_version_message(),
        };
...
mod tests {
    ...
    async fn it_works() {
        let mut peer = Peer::connect(
            "localhost:22556",
            BitcoinCompatibleNetwork::Dogecoin.magic(),
        )
        .await
        .unwrap();

        let hdrs = peer
            .get_new_headers(vec![BlockHash::from_str(
                "eb19d2333149818abc07f80478554ecb52ab6183c6235c621e2264cacdf3e89c",
            )
            .unwrap()])
            .await
            .unwrap();

        peer.get_blocks(hdrs.into_iter().map(|x| x.block_hash()).collect())
            .await
            .unwrap();
    }
}
